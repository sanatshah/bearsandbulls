import { jsxs as _jsxs, jsx as _jsx } from "frog/jsx/jsx-runtime";
...
export function routes(app, path) {
    app
        .use(`${parsePath(path)}/dev`, (c, next) => jsxRenderer((props) => {
        const { children } = props;
        const path = new URL(c.req.url).pathname.replace('/dev', '');
        return (_jsxs("html", { lang: "en", children: [_jsxs("head", { children: [_jsxs("title", { children: ["frame: ", path || '/'] }), _jsx(Fonts, {}), _jsx(Styles, {}), _jsx(Scripts, {}), _jsx("link", { rel: "icon", href: "https://frog.fm/icon.png", type: "image/png" })] }), _jsx("body", { children: children })] }));
    })(c, next))
        .get(async (c) => {
    ...
    app
        ...
        .post(`${parsePath(path)}/dev/frame/action`, validator('json', validateFramePostBody), async (c) => {
        // TODO: Hono should be able to infer these types
        const vars = c.var;
        const json = c.req.valid('json');
        const fid = (json.fid ?? vars.fid);
        const body = { ...json, fid };
        ...
        try {
            const state = JSON.parse(decodeURIComponent(frame.state)).previousState;
            if (state)
                if (app.secret)
                    frame.debug.state = JSON.parse(await verify(state, app.secret));
                else
                    frame.debug.state = state;
        }
        catch (error) { }
        // remove serialized context from image/imageUrl to save url space
        // tip: search for `_frog_` to see where it's added back
        ...
        .post(`${parsePath(path)}/dev/frame/redirect`, validator('json', validateFramePostBody), async (c) => {
        // TODO: Hono should be able to infer these types
        const vars = c.var;
        const json = c.req.valid('json');
        const fid = (json.fid ?? vars.fid);
    ...
    app
        ...
        if (app.dev?.appFid && app.dev?.appMnemonic) {
            ...
            const response = (await fetch(`https://auth.frog.fm/api/signed-key-requests/${publicKey}`, {
                method: 'GET',
        ...
        const rendered = await c.render(_jsx(QRCode, { url: url }));
        ...
        if (app.secret)
            ...
            setCookie(c, 'session', value, { ...cookieOptions, httpOnly: true });
        ...
        if (state === 'completed') {
            ...
            if (app.hubApiUrl) {
                const response = (await fetch(`${app.hubApiUrl}/v1/userDataByFid?fid=${userFid}`).then((response) => response.json()));
